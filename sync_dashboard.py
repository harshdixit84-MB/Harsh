"""
Scheduled job: fetches current matches from a Chartlink screener, filters
out ETFs/indices and low-volume stocks, then merges the results into a
Google Sheet -- preserving manually-set buy targets and auto-archiving
...
def sync_stocks_to_sheet(sheet, fetched_stocks):
    ...
    print(f"Synced {len(updated_rows)} total rows ({len(fetched_by_symbol)} from today's fetch).")
